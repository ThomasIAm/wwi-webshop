		...
		if(isset($_POST['tos'])){
			...
			if($emailduplicate == ""){
				if($passwordplain == $passwordconfirm){
					...
					if(isset($_POST['Register'])){
                                            //er wordt een nieuw account aangemaakt als er geen bestaande email bij de ingevulde gegevens zitten
						...
						if($firstname == "" || $lastname == "" || $email == "" || $passwordplain == "" || $country == "" || $province == "" || $townCity == "" || $address == "" || $zip == "" || $phone == ""){
							...
							if (filter_has_var(INPUT_GET, 'stockKey')) {
								redirect('productPagina.php?stockKey='.filter_input(INPUT_GET, 'stockKey', FILTER_SANITIZE_NUMBER_INT));
							} else {
								redirect('loginhead.php');
							}
						} else {
							print("<h5 class='' style='color: red;'>Please fill out all fields</h5>");
						}
				...
				if($passwordplain == $passwordconfirm){
					$checkedpassword = $passwordplain;
					if(isset($_POST['Register'])){
                                            //Hier word een gast account verandert in een echt account als de overeenkomende email in de database geen wachtwoord bevat
