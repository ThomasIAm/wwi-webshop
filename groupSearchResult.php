...
//wordt gebruikt om te checken welke productgroep er geladen moet worden
$stockGroupID = filter_input(INPUT_GET, "StockGroupID", FILTER_SANITIZE_NUMBER_INT);
...
//Group Zoek resultaten Queries
$query = "SELECT G.StockItemID, S.StockItemName, S.UnitPrice FROM stockitemstockgroups AS G JOIN stockitems AS S ON G.StockItemID = S.StockItemID WHERE G.StockGroupID = $stockGroupID ORDER BY $order";

$zoekResultatenTeller = $query.";";

$test = $query." LIMIT $momenteelGeladen";
?>
    <!-- Page Content -->
    <div class="container">
        <div class="row">
            <div class="col-lg-3">
                ...
                <ul class="list-group">
                    ...
                    <form action="" method="get">
						<div class="row">
							<div class="col">
								<select style="width: 170px;" name="orderby" class="form-control" >
							...
							<div class="col">
								<?php
								echo '<input type="hidden" name="StockGroupID" value="'.$stockGroupID.'">';
								?>
					...
					<br><br>
                    <?php
                    //laadt de groepzoekmachine
                    foreach ($db->query($groupName) as $row) {
                        if ($stockVar == $stockGroupID) {
                            echo "<li class='list-group-item'>";
                            echo '<form action="groupSearchResult.php" method="get">';
                            echo '<button style="color: #69D2F4" type="submit" name="StockGroupID" value="'.$stockVar.'"><h6><img style="height: 10px;" src="img/arrow.png" alt="">'.$row['StockGroupName'].'</h6></button></form>';
                            echo "</li>";
                            $stockVar++;
                        } else {
                            echo "<li class='list-group-item'>";
            ...
            <div class="col-lg-9">
                ...
                $idcounter = 1;
                foreach ($db->query($test)as $row) {
                    //data van de zoekresultaten wordt geladen
                    ...
                    echo '<input type="hidden" name="stockKey" value='.$row["StockItemID"].'>
                          <input type="hidden" name="StockGroupID" value='.$stockGroupID.'>
